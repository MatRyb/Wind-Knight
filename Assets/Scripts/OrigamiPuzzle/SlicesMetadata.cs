...
public class SlicesMetadata
{
    ...
    public SlicesMetadata(Plane plane, Mesh mesh, bool isSolid, bool createReverseTriangleWindings, bool shareVertices, bool smoothVertices)
    {
        _plane = plane;
        _mesh = mesh;
        _positiveSideTriangles = new List<int>[_mesh.subMeshCount];
        for (int i = 0; i < _mesh.subMeshCount; i++)
        {
            _positiveSideTriangles[i] = new List<int>();
        }
        _positiveSideVertices = new List<Vector3>();
        _negativeSideTriangles = new List<int>[_mesh.subMeshCount];
        for (int i = 0; i < _mesh.subMeshCount; i++)
        {
            _negativeSideTriangles[i] = new List<int>();
        }
    ...
    private void AddTrianglesNormalAndUvs(MeshSide side, Vector3 vertex1, Vector3? normal1, Vector2 uv1, Vector3 vertex2, Vector3? normal2, Vector2 uv2, Vector3 vertex3, Vector3? normal3, Vector2 uv3, bool shareVertices, bool addFirst, int subMesh)
    {
        List<int> subMeshTriangles;
        if (side == MeshSide.Positive)
        {
            subMeshTriangles = _positiveSideTriangles[subMesh];
            AddTrianglesNormalsAndUvs(ref _positiveSideVertices, ref subMeshTriangles, ref _positiveSideNormals, ref _positiveSideUvs, vertex1, normal1, uv1, vertex2, normal2, uv2, vertex3, normal3, uv3, shareVertices, addFirst);
        }
        else
        {
            subMeshTriangles = _negativeSideTriangles[subMesh];
            AddTrianglesNormalsAndUvs(ref _negativeSideVertices, ref subMeshTriangles, ref _negativeSideNormals, ref _negativeSideUvs, vertex1, normal1, uv1, vertex2, normal2, uv2, vertex3, normal3, uv3, shareVertices, addFirst);
        }
    }
    ...
    private void AddReverseTriangleWinding()
    {
        ...
        int numPositiveTrianglesSubMeshes = _positiveSideTriangles.Length;

        for (int s = 0; s < numPositiveTrianglesSubMeshes; s++)
        {
            int numPositiveTriangles = _positiveSideTriangles[s].Count;

            //Add reverse windings
            for (int i = 0; i < numPositiveTriangles; i += 3)
            {
                _positiveSideTriangles[s].Add(positiveVertsStartIndex + _positiveSideTriangles[s][i]);
                _positiveSideTriangles[s].Add(positiveVertsStartIndex + _positiveSideTriangles[s][i + 2]);
                _positiveSideTriangles[s].Add(positiveVertsStartIndex + _positiveSideTriangles[s][i + 1]);
            }
        }
        ...
        int numNegativeTrianglesSubMeshes = _negativeSideTriangles.Length;

        for (int s = 0; s < numNegativeTrianglesSubMeshes; s++)
        {
            int numNegativeTriangles = _negativeSideTriangles[s].Count;

            //Add reverse windings
            for (int i = 0; i < numNegativeTriangles; i += 3)
            {
                _negativeSideTriangles[s].Add(negativeVertextStartIndex + _negativeSideTriangles[s][i]);
                _negativeSideTriangles[s].Add(negativeVertextStartIndex + _negativeSideTriangles[s][i + 2]);
                _negativeSideTriangles[s].Add(negativeVertextStartIndex + _negativeSideTriangles[s][i + 1]);
            }
        }
    }
    ...
    private void JoinPointsAlongPlane()
    {
        ...
        for (int i = 0; i < _pointsAlongPlane.Count; i += 2)
        {
            ...
            if (direction > 0)
            {
                AddTrianglesNormalAndUvs(MeshSide.Positive, halfway, -normal3, Vector2.zero, firstVertex, -normal3, Vector2.zero, secondVertex, -normal3, Vector2.zero, false, true, 0);
                AddTrianglesNormalAndUvs(MeshSide.Negative, halfway, normal3, Vector2.zero, secondVertex, normal3, Vector2.zero, firstVertex, normal3, Vector2.zero, false, true, 0);
            }
            else
            {
                AddTrianglesNormalAndUvs(MeshSide.Positive, halfway, normal3, Vector2.zero, secondVertex, normal3, Vector2.zero, firstVertex, normal3, Vector2.zero, false, true, 0);
                AddTrianglesNormalAndUvs(MeshSide.Negative, halfway, -normal3, Vector2.zero, firstVertex, -normal3, Vector2.zero, secondVertex, -normal3, Vector2.zero, false, true, 0);
            }
        }
    }
    ...
    private void SetMeshData(MeshSide side)
    {
        if (side == MeshSide.Positive)
        {
            ...
            _positiveSideMesh.subMeshCount = _positiveSideTriangles.Length;
            for (int i = 0; i < _positiveSideTriangles.Length; i++)
            {
        ...
        else
        {
            ...
            _negativeSideMesh.subMeshCount = _negativeSideTriangles.Length;
            for (int i = 0; i < _negativeSideTriangles.Length; i++)
            {
    ...
    private void ComputeNewMeshes()
    {
        ...
        int[][] meshTriangles = new int[subMeshCount][];
        for (int i = 0; i < subMeshCount; i++)
        {
            meshTriangles[i] = _mesh.GetTriangles(i);
        }
        ...
        for (int s = 0; s < subMeshCount; s++)
        {
            for (int i = 0; i < meshTriangles[s].Length; i += 3)
            {
                ...
                Vector3 vert2 = meshVerts[meshTriangles[s][i + 1]];
                ...
                Vector3 vert3 = meshVerts[meshTriangles[s][i + 2]];
                ...
                if (vert1Side == vert2Side && vert2Side == vert3Side)
                {
                    ...
                    AddTrianglesNormalAndUvs(side, vert1, normal1, uv1, vert2, normal2, uv2, vert3, normal3, uv3, true, false, s);
                }
                else
                {
                    ...
                    if (vert1Side == vert2Side)
                    {
                        ...
                        AddTrianglesNormalAndUvs(side1, vert1, null, uv1, vert2, null, uv2, intersection1, null, intersection1Uv, _useSharedVertices, false, s);
                        AddTrianglesNormalAndUvs(side1, vert1, null, uv1, intersection1, null, intersection1Uv, intersection2, null, intersection2Uv, _useSharedVertices, false, s);

                        AddTrianglesNormalAndUvs(side2, intersection1, null, intersection1Uv, vert3, null, uv3, intersection2, null, intersection2Uv, _useSharedVertices, false, s);
                    ...
                    else if (vert1Side == vert3Side)
                    {
                        ...
                        AddTrianglesNormalAndUvs(side1, vert1, null, uv1, intersection1, null, intersection1Uv, vert3, null, uv3, _useSharedVertices, false, s);
                        AddTrianglesNormalAndUvs(side1, intersection1, null, intersection1Uv, intersection2, null, intersection2Uv, vert3, null, uv3, _useSharedVertices, false, s);

                        AddTrianglesNormalAndUvs(side2, intersection1, null, intersection1Uv, vert2, null, uv2, intersection2, null, intersection2Uv, _useSharedVertices, false, s);
                    }
                    //Vert1 is alone
                    else
                    {
                        ...
                        AddTrianglesNormalAndUvs(side2, intersection1, null, intersection1Uv, vert2, null, uv2, vert3, null, uv3, _useSharedVertices, false, s);
                        AddTrianglesNormalAndUvs(side2, intersection1, null, intersection1Uv, vert3, null, uv3, intersection2, null, intersection2Uv, _useSharedVertices, false, s);
                    }
    ...
    private void DoSmoothing(ref List<Vector3> vertices, ref List<Vector3> normals, ref List<int>[] triangles)
    {
        ...
        {
            for (int i = 0; i < triangles[s].Count; i += 3)
            {
                int vertIndex1 = triangles[s][i];
                int vertIndex2 = triangles[s][i + 1];
                int vertIndex3 = triangles[s][i + 2];
